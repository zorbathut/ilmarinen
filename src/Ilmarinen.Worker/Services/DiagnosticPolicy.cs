...
/// <summary>
/// When a worker runs another diagnostic on its own initiative. What a report means for taking jobs is
/// <see cref="DiagnosticStatusPolicy"/>, which the server shares.
/// </summary>
internal static class DiagnosticPolicy
{
    ...
    public static bool ShouldRetry(DiagnosticReport? lastReport, bool connected, bool draining)
    {
        return lastReport != null
            && !DiagnosticStatusPolicy.CanAcceptJobs(lastReport.Status)
            && connected
            && !draining;
    }
