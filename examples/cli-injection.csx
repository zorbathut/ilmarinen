...
Step("test-cli")
    ...
    .Run(async ctx =>
    {
        ...
        var result = await ctx.Shell("which ilmarinen");
        AssertContains(result.Stdout, "/usr/local/bin/ilmarinen", "CLI not found at expected path");

        result = await ctx.Shell("ilmarinen --version");
        AssertContains(result.Stdout, "ilmarinen", "Version output missing ilmarinen");

        // Test help
        Console.WriteLine("2. Testing ilmarinen --help...");
        result = await ctx.Shell("ilmarinen --help");
...
Step("test-cli-nested")
    ...
    .Run(async ctx =>
    {
        ...
        var result = await ctx.Shell("ilmarinen run alpine -- echo 'Hello from nested!'");
        ...
        result = await ctx.Shell("ilmarinen run alpine -- cat /workspace/cli-test.txt");
